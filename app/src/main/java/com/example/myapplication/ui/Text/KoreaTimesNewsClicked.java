...
public class KoreaTimesNewsClicked extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        KoreaTimesNewsClicked.JsoupAsyncTask jsoupAsyncTask = new KoreaTimesNewsClicked.JsoupAsyncTask();
    ...
    private class JsoupAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                Document doc = Jsoup.connect("https://www.koreatimes.co.kr"+url).get();
                Elements news= doc.select("div[id=startts]").select("span");
                for(Element e: news){
                    body += e.text().trim()+"\n";
                }
            } catch (IOException e) {
